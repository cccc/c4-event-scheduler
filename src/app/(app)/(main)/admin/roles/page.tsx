...
export default function AdminRolesPage() {
	...
	const { data: users, isLoading: usersLoading } = api.roles.listUsers.useQuery(
		undefined,
		{ enabled: isAdmin },
	);
	...
	return (
		<>
			<div className="mb-8">
				<h1 className="mb-2 font-bold text-3xl">Permission Management</h1>
				<p className="text-muted-foreground">
					Manage user permissions for spaces and event types.
				</p>
				<p className="mt-2 text-muted-foreground text-sm">
					Permissions from OIDC are synced automatically on login. Manual
						...
						<div className="rounded-lg border p-4" key={u.id}>
							...
							{u.permissions.length > 0 ? (
								...
								<p className="text-muted-foreground text-sm">
									No specific permissions (requires admin or OIDC claims)
								</p>
							)}
						</div>
					))}
