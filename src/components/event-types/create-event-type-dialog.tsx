...
import { create as createEventTypeFn } from "@/server/fns/event-types";
...
export function CreateEventTypeDialog({
    ...
    const createEventType = useMutation({
        mutationFn: (input: Parameters<typeof createEventTypeFn>[0]["data"]) =>
            createEventTypeFn({ data: input }),
        onSuccess: () => {
        ...
        onSubmit: async ({ value }) => {
            createEventType.mutate({
                ...
                defaultDurationMinutes: value.defaultDurationMinutes
                    ...
                    : undefined,
                spaceId:
                    value.spaceId === GLOBAL_VALUE ? undefined : value.spaceId,
            });
        },
    });
