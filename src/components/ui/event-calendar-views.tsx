...
//   - today's cell/lane uses the app accent instead of a yellow tint (getDayClass)
...
const getDayClass = (info: {
    ...
    cn(
        ...
        info.isDisabled
            ? "bg-foreground/3"
            : info.isToday && "not-print:bg-accent", // c4: app accent instead of yellow
    );

// c4: today's date number (month view) and today's column header (week view)
...
const dayRowCommonClasses: CalendarOptions = {
  ...
  ----------------------------------------------------------------------------------------------- */
    ...
    listItemEventBeforeClass: (info) =>
        cn("border-4", info.isNarrow ? "mx-px" : "mx-1"),
  ...
  ----------------------------------------------------------------------------------------------- */
    ...
    rowMoreLinkInnerClass: (info) =>
        cn("p-px", info.isNarrow ? xxsTextClass : "text-xs"),
};
...
export function EventCalendarViews({
    ...
    return (
      ...
      ----------------------------------------------------------------------------------------- */
            ...
            // c4: bordered, tinted block instead of a solid fill; the event-* rules in
            // globals.css vary border style and tint per status on top of this
            blockEventClass={(info) =>
                cn(
                    "group relative rounded border-2 border-(--fc-event-color) bg-[color-mix(in_srgb,var(--fc-event-color)_20%,transparent)] print:bg-white",
      ...
      ----------------------------------------------------------------------------------------- */
            ...
            columnMoreLinkInnerClass={(info) =>
                cn("p-0.5", info.isNarrow ? xxsTextClass : "text-xs")
            }
            /* Day Header
      ----------------------------------------------------------------------------------------- */
            ...
            dayHeaderInnerClass={(info) =>
                cn(
                    ...
                    info.isToday && !info.inPopover && todayPillClass, // c4
                )
            }
      ...
      ----------------------------------------------------------------------------------------- */
            ...
            dayCellTopInnerClass={(info) =>
                cn(
                    ...
                    info.isToday && todayPillClass, // c4
                )
            }
      ...
      ----------------------------------------------------------------------------------------- */
            ...
            views={{
                ...
                multiMonth: {
                    ...
                    dayCellBottomClass: "min-h-px",
                    tableClass: (info) =>
                        cn(info.multiMonthColumns > 1 && "border"),
          ...
          ------------------------------------------------------------------------------------- */
                    ...
                    slotHeaderInnerClass: (info) =>
                        cn(
                            "mx-1 my-0.5 text-muted-foreground", // c4: muted time axis
                            info.isNarrow ? xxsTextClass : "text-sm",
                        ),
          ...
          ------------------------------------------------------------------------------------- */
                    ...
                    nowIndicatorHeaderClass:
                        "start-0 -mt-[5px] border-y-[5px] border-y-transparent border-s-[6px] border-s-destructive",
                ...
                list: {
                    listDayClass: (info) =>
                        cn(!info.isLast && "border-b", "flex flex-col"),
          ...
          ------------------------------------------------------------------------------------- */
                    ...
                    noEventsClass:
                        "bg-foreground/5 flex flex-col items-center justify-center",
