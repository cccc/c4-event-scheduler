                ...
                if (!evt.rrule) {
                    ...
                    try {
                        ...
                        for (const date of allDates) {
                            ...
                            if (
                                exdatesSet.has(occDate) &&
                                !input.includeExdates
                            )
                                continue;
                            ...
                            if (start < input.start || start > input.end)
                                continue;
    ...
    create: protectedProcedure
        ...
        .use(async ({ ctx, input, next }) => {
            const [spaceRec, etRec] = await Promise.all([
                ctx.db.query.space.findFirst({
                    where: eq(space.id, input.spaceId),
                }),
                ctx.db.query.eventType.findFirst({
            ...
            return next({ ctx: { spaceRec, etRec } });
        })
        ...
        .mutation(async ({ ctx, input }) => {
            ...
            const [result] = await ctx.db
                ...
                .set({
                    ...
                    sequence: ctx.existingEvent.sequence + 1,
                    updatedAt: new Date(),
            ...
            await ctx.db
                ...
                .where(
                    and(
                        ...
                        eq(
                            occurrenceOverride.occurrenceDate,
