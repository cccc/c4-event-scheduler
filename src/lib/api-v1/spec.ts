...
const authNote =
	"When no valid API key is provided, draft events are hidden and author fields (`createdBy`, `updatedBy`, `createdByApiKey`, `updatedByApiKey`, `createdById`, `updatedById`, `createdByApiKeyId`, `updatedByApiKeyId`) are omitted from all responses.";
...
const errorResponses = {
	...
	403: {
		description: "Forbidden — the API key lacks permission for this resource",
		content: { "application/json": { schema: ErrorSchema } },
	},
	404: {
	...
	request: {
		query: z.object({
			spaceSlug: z
				.string()
				.optional()
				.openapi({ description: "Filter by space slug", example: "my-space" }),
			...
			limit: z
				.string()
				.optional()
				.openapi({ description: "Max results per page (1–100, default 50)" }),
...
registry.registerPath({
	...
	responses: {
		200: {
			...
			content: {
				"application/json": { schema: z.object({ data: EventDetailSchema }) },
			},
		},
		...errorResponses,
	},
...
registry.registerPath({
	...
	responses: {
		200: {
			...
			content: {
				"application/json": {
					schema: z.object({ data: z.object({ success: z.literal(true) }) }),
				},
			},
		},
		...errorResponses,
